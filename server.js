...
app.post('/api/login', async (req, res) => {
    ...
    db.get("SELECT * FROM users WHERE username = ?", [username], async (err, user) => {
        ...
        db.all("SELECT device_id FROM user_devices WHERE user_id = ?", [user.id], (err, devices) => {
            ...
            const bypassDeviceLimit = user.is_admin === 1 || user.is_owner === 1;
            const currentDeviceCount = deviceRows.length;
            const isDeviceBound = deviceRows.some(d => d.device_id === deviceId);
            if (!bypassDeviceLimit && !isDeviceBound && currentDeviceCount >= user.device_limit) {
